...
def sequence_alignment_efficient(s1, s2):
    """Memory-efficient sequence alignment using divide and conquer approach"""
    m, n = len(s1), len(s2)
    ...
    # Use basic algorithm for small inputs
    if min(m, n) <= 3:
    ...
    split_j = total_costs.index(min(total_costs))
...
def compute_costs_forward(s1, s2):
    """Compute alignment costs left to right using space-optimized DP"""
    m, n = len(s1), len(s2)
...
def compute_costs_backward(s1, s2):
    """Compute alignment costs right to left using space-optimized DP"""
    m, n = len(s1), len(s2)
...
def sequence_alignment_basic(s1, s2):
    """Basic DP algorithm for small inputs"""
    m, n = len(s1), len(s2)
...
def main():
    ...
    # Run efficient alignment with performance measurement
    def alignment_func():
